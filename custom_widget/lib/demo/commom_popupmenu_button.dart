...
import 'package:custom_widget/widget/custom_popup_menu.dart';
...
class CommonPopupMenuButton extends StatelessWidget {
  ...
  List<CustomPopupMenuEntry<String>> _buildPopupMenuItems(
      BuildContext context) {

    List<CustomPopupMenuEntry<String>> itemViews =
        <CustomPopupMenuEntry<String>>[];
    List.generate(items.length, (int index) {
      CommonPopupMenuItem item = items.elementAt(index);
      itemViews.add(CustomPopupMenuItem<String>(
      ...
      if (index < items.length - 1) {
        itemViews.add(CustomPopupMenuDivider(
          padding: EdgeInsets.only(
            left: Adaptive.width(10),
            right: Adaptive.width(10),
